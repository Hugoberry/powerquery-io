...
Vrátí hodnotu `Guid.Type` z dané `value`. Pokud dané `value` je `null`, `Guid.From` vrátí `null`. Bude provedena kontrola, jestli je dané `value` v přijatelném formátu. Přípustné formáty jsou uvedeny v příkladech.
