...
Trả về một bảng chứa dạng xem và bảng SQL có trong cơ sở dữ liệu Informix trên máy chủ `server` trong phiên bản cơ sở dữ liệu có tên là `database`. Có thể tùy ý chỉ định cổng với máy chủ, được phân tách bằng dấu hai chấm. Có thể chỉ định tham số bản ghi tùy chọn, `options`, để kiểm soát các tùy chọn sau đây:
