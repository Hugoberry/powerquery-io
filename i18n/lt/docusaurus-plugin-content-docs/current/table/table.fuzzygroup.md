...
Grupuojamos `table` eilutės pagal apytikslį reikšmių, pateikiamų nurodytame stulpelyje, `key`, kiekvienoje eilutėje, derinimą. Kiekvienai grupei įrašas yra sudarytas su raktų stulpeliais (ir jų reikšmėmis) kartu su visais agreguotais stulpeliais, nurodytais `aggregatedColumns`. Šia funkcija negarantuojama, kad bus pateikta ištaisyta eilučių tvarka.

Gali būti įtrauktas pasirinktinis `options` rinkinys, nurodantis, kaip lyginti raktų stulpelius. Parinktys apima:
...
### Example #1
Grupuokite lentelę pridėdami agreguotą stulpelį \[Count\], kuriame yra darbuotojų skaičius kiekvienoje vietoje (`each Table.RowCount(_)`).
```powerquery
Table.FuzzyGroup(
    Table.FromRecords(
        {
