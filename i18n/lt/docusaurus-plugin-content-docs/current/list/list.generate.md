...
Naudojant pateiktas funkcijas generuojamas reikšmių sąrašas. Funkcija `initial` sugeneruoja pradinę potencialią reikšmę, kuri yra tikrinama pagal `condition`. Jei potenciali reikšmė patvirtinama, ji pateikiama kaip gauto sąrašo dalis, o kita potenciali reikšmė sugeneruojama naujai patvirtintą reikšmę perduodant į `next`. Kai potenciali reikšmė neatitinka `condition`, sąrašo generavimo procesas sustoja. Taip pat gali būti pateiktas pasirenkamas parametras, `selector`, kad būtų transformuoti elementai gautame sąraše.
