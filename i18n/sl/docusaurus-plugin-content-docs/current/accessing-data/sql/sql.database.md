...
Vrne tabelo s tabelami, pogledi in shranjenimi skalarnimi funkcijami SQL, ki so na voljo v zbirki podatkov strežnika SQL Server `database` v strežniku `server`. S strežnikom lahko izbirno določite vrata, ki jih ločite z dvopičjem ali vejico. Navedete lahko izbirni parameter zapisa `options`, s katerim nadzorujete te možnosti:
