...
ส่งกลับค่า `Guid.Type` จาก `value` ที่กำหนด ถ้า `value` ที่กำหนดเป็น `null` `Guid.From` จะส่งกลับค่า `null` จะมีดำเนินการตรวจสอบเพื่อกำหนดว่า `value` ที่กำหนดอยู่ในรูปแบบที่ยอมรับได้หรือไม่ รูปแบบที่ยอมรับได้มีอยู่ในตัวอย่าง
