...
Retourne une table des tables SQL et des onglets depuis la base de données Teradata.
...
Retourne une table des tables SQL et des vues depuis la base de données Teradata sur le serveur `server`. Le port peut être spécifié en option avec le serveur, séparé par deux-points. Un paramètre d'enregistrement facultatif `options` peut être spécifié pour contrôler les options suivantes :
