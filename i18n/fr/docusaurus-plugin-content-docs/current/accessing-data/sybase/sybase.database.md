...
Retourne une table des tables SQL et des onglets disponibles dans une base de données Sybase.
...
Renvoie une table des tables et vues SQL disponibles dans une base de données Sybase sur le serveur `server` au sein de l'instance de base de données nommée `database`. Le port peut être spécifié en option avec le serveur, séparé par deux-points. Un paramètre d'enregistrement facultatif `options` peut être spécifié pour contrôler les options suivantes :
