...
Retourne une table des tables SQL et des onglets depuis la base de données Oracle.
...
Renvoie une table des tables et vues SQL de la base de données Oracle sur le serveur `server`. Le port peut être spécifié en option avec le serveur, séparé par deux-points. Un paramètre d'enregistrement facultatif `options` peut être spécifié pour contrôler les options suivantes :
