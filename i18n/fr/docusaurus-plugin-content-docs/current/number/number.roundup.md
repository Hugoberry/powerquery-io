...
Renvoie le résultat de l'arrondi `number` à l'entier supérieur suivant. Si `number` est null, cette fonction renvoie null. Si `digits` est fourni, `number` est arrondi au nombre de chiffres décimaux spécifié.
