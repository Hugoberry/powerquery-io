...
Teradata データベースの SQL テーブルとビューのテーブルを返します。
...
サーバー `server` 上の Teradata データベースの SQL テーブルとビューのテーブルを返します。ポートはオプションで、サーバー名の後にコロンで区切って指定できます。オプションのレコード パラメーター `options` を指定して、次のオプションを制御できます:
