...
Agrupa as linhas de `table` pela correspondência difusa de valores na coluna especificada, `key`, para cada linha. Para cada grupo, um registro é construído contendo as colunas de chave (e seus valores) juntamente com as colunas agregadas especificadas por `aggregatedColumns`. Essa função não pode garantir o retorno de uma ordem fixa de linhas.
...
### Example #1
Agrupar a tabela adicionando uma coluna de agregação \[Count\] que contém o número de funcionários em cada local (`each Table.RowCount(_)`).
```powerquery
Table.FuzzyGroup(
    Table.FromRecords(
        {
