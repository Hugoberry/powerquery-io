...
Retorna um registro de erro a partir dos valores de texto fornecidos para motivo, mensagem, detalhes e código de erro.
...
-   `detail`: (Opcional) Informações adicionais detalhadas sobre o erro.
-   `parameters`: (Opcional) Uma lista de valores que fornecem contexto adicional para o erro, normalmente usada para diagnósticos ou tratamento programático.
...
### Example #1
Trate um erro de divisão por zero.
```powerquery
let
    input = 100,
    divisor = 0,
...
### Example #2
Trate uma entrada com erro de ID de cliente inexistente. Se nenhum erro ocorrer, indique uma entrada bem-sucedida.
```powerquery
let
    CustomerId = 12345,
