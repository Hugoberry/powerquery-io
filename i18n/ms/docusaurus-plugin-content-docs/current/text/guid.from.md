...
Mengembalikan nilai `Guid.Type` daripada `value` yang ditentukan. Jika `value` yang ditentukan adalah `nol`, `Guid.From` mengembalikan `nol`. Pemeriksaan akan dilaksanakan untuk menentukan jika `value` yang ditentukan berada dalam format yang boleh diterima. Format yang boleh diterima diberikan dalam contoh.
