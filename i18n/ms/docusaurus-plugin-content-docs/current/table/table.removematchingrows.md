...
Mengalih keluar semua kejadian bagi baris yang ditentukan daripada jadual.

-   `table`: Jadual untuk dicari.
-   `rows`: Senarai yang mengandungi maklumat tentang baris yang akan dialih keluar.
-   `equationCriteria`: (Pilihan) Menentukan cara persamaan ditentukan apabila membandingkan nilai. Parameter ini boleh menjadi fungsi pemilih kunci, fungsi pembanding, atau senarai lajur dalam jadual yang digunakan apabila membandingkan baris.


## Examples

### Example #1
Alih keluar sebarang baris yang \[a = 1\] daripada jadual yang ditentukan.
```powerquery
Table.RemoveMatchingRows(
    Table.FromRecords({
        [a = 1, b = 2],
...
### Example #2
Alih keluar pesanan yang dibatalkan tanpa mengira huruf besar atau kecil.
```powerquery
let
...
### Example #3
Alih keluar sebarang tugas penyelenggaraan yang jatuh pada hari cuti di AS.
```powerquery
let
