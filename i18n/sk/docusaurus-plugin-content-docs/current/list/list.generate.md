...
Vygeneruje zoznam hodnôt pomocou poskytnutých funkcií. Funkcia `initial` vygeneruje počiatočnú hodnotu kandidáta, ktorá sa potom testuje na základe `condition`. Ak je kandidátska hodnota schválená, vráti sa ako súčasť výsledného zoznamu a ďalšia kandidátska hodnota sa vygeneruje odovzdaním novo schválenej hodnoty do `next`. Keď sa hodnote kandidáta nepodarí zhodovať `condition`, proces generovania zoznamu sa zastaví. Na transformáciu položiek vo výslednom zozname možno zadať voliteľný parameter `selector`.
