...
-   `rows`: Zoznam obsahujúci informácie o riadkoch, ktoré sa majú odstrániť.
-   `equationCriteria`: (Voliteľné) Určuje, ako sa určuje rovnosť pri porovnávaní hodnôt. Tento parameter môže byť funkcia výberu kľúča, funkcia porovnávača alebo zoznam stĺpcov v tabuľke, ktoré sa majú použiť pri porovnávaní riadkov.


## Examples

### Example #1
Odstrániť zo zadanej tabuľky všetky riadky, kde platí \[a = 1\].
```powerquery
Table.RemoveMatchingRows(
    Table.FromRecords({
        [a = 1, b = 2],
...
### Example #2
Odstrániť zrušené objednávky bez ohľadu na veľkosť písmen.
```powerquery
let
...
### Example #3
Odstrániť všetky úlohy údržby, ktoré pripadajú na štátne sviatky v USA.
```powerquery
let
