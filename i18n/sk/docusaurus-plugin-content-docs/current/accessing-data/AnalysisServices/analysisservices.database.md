...
Vráti tabuľku multidimenzionálnych kociek alebo tabuľkových modelov z databázy služby Analysis Services `database` na serveri `server`. Môžete zadať voliteľný parameter záznamu `options`, ktorý bude ovládať nasledujúce možnosti:

-   `Query` : Natívny dotaz MDX slúžiaci na načítanie údajov.
