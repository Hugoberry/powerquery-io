...
从服务器 `server` 上的 Netezza 服务器数据库 `database` 中返回 Netezza 表、视图和存储函数的表。可根据需要指定服务器的端口，并用冒号分隔。可以指定可选的记录参数 `options` 来控制以下选项:
...
-   `NormalizeDatabaseName`: 一个逻辑值(true/false)，用于设置是否将数据库名称规范化为大写或按字面意义解释(默认值为 true)
