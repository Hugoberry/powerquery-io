...
以二进制形式返回从 `url` 下载的内容。可以提供可选记录参数 `options` 来指定其他属性。记录可包含以下字段:
...
### Example #1
使用 RelativePath 和“查询”选项检索 `"https://bing.com/search?q=Power+Query"` 的内容。这些选项可用于动态查询静态基 URL。
```powerquery
let
...
### Example #3
连接到接受身份验证密钥作为其查询字符串一部分的安全 URL。密钥并非通过在 M 中对密钥进行硬编码来提供(这回带来安全风险)， 而是通过在 M 中指定密钥名称(而不是其值)、选择 Web API 身份验证，并输入密钥值作为 Web API 凭据的一部分来实现。 以这种方式使用时，以下示例将生成对 `"https://contoso.com/api/customers/get?api_key=******"` 的请求。
```powerquery
