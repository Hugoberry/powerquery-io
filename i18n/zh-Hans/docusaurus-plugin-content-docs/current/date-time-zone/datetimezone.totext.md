...
返回 `dateTimeZone` 的文本表示形式。可提供可选的 `record` 参数 `options`，用于指定其他属性。`culture` 仅用于旧版工作流。`record` 可包含以下字段:
...
### Example #1
将 `#datetimezone(2010, 12, 31, 01, 30, 25, 2, 0)` 转换为 `text` 值。*结果输出可能因当前区域性而异。*
```powerquery
