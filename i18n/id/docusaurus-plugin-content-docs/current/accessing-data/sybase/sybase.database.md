...
Menghasilkan tabel berisi tabel dan tampilan SQL yang tersedia di database Sybase di server `server` dalam instans database `database`. Port mungkin ditentukan secara opsional dengan server, dipisahkan oleh titik dua. Parameter catatan opsional, `options`, dapat ditentukan untuk mengontrol opsi berikut:
