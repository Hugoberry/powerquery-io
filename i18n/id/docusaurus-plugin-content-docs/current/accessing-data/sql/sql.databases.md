...
Menghasilkan tabel database pada SQL server tertentu, `server`. Parameter catatan opsional, `options`, dapat ditentukan untuk mengontrol opsi berikut:
...
Tidak mendukung pengaturan kueri SQL untuk dijalankan di server. `Sql.Database` harus digunakan sebagai gantinya untuk menjalankan kueri SQL.
