...
Mengkonversi daftar `list` menjadi tabel dengan menerapkan fungsi pemisah opsional, `splitter`, pada setiap item dalam daftar. Secara default, daftar tersebut diperkirakan sebagai daftar nilai teks yang dibagi dengan koma. `columns` opsional dapat berupa jumlah kolom, daftar kolom, atau TableType. `default` dan `extraValues` opsional juga dapat ditetapkan.
