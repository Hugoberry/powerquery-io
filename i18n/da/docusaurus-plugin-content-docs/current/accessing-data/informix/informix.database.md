...
Returnerer en tabel over SQL-tabeller og visninger, der er tilgængelige i en Informix-database på serveren `server` i den forekomst af databasen, der hedder `database`. Porten kan eventuelt være angivet med serveren, adskilt af et kolon. En valgfri parameter for post, `options`, kan være angivet for at kontrollere følgende indstillinger:
