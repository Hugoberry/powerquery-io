...
Hiermee wordt een tabel met SQL-tabellen en -weergaven uit de OLE DB-gegevensbron geretourneerd die is opgegeven in de verbindingsreeks `connectionString`. `connectionString` kan tekst zijn of een lijst met waarden en eigenschappen. Eigenschapswaarden kunnen tekst of cijfers zijn. Er kan een optionele recordparameter, `options`, worden opgegeven om aanvullende eigenschappen op te geven. De record kan de volgende velden bevatten:
...
-   `HierarchicalNavigation` : Een logische waarde (true/false) waarmee wordt ingesteld of de tabellen gegroepeerd op de schemanamen moeten worden weergegeven (de standaardinstelling is true).
