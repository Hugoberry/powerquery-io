...
Retourneert een tabel met SQL-tabellen en weergaven die beschikbaar zijn in een Sybase-database.
...
Hiermee wordt een tabel met SQL-tabellen en weergaven geretourneerd die beschikbaar zijn in een Sybase-database op server `server` in de database-instantie met de naam `database`. De poort kan optioneel worden opgegeven bij de server, gescheiden door een dubbele punt. U kunt een optionele recordparameter `options` opgeven voor besturing van de volgende opties:
