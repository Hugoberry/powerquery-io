...
Hiermee wordt een tabel met SQL-tabellen en weergaven geretourneerd die beschikbaar zijn in een Oracle-database op server `server`. De poort kan optioneel worden opgegeven bij de server, gescheiden door een dubbele punt. U kunt een optionele recordparameter `options` opgeven voor besturing van de volgende opties:
