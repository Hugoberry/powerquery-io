...
Retourneert het resultaat van het afronden van `number` op basis van het teken bij het getal. Deze functie rondt positieve getallen naar boven en negatieve getallen naar beneden af. Als `digits` is opgegeven, wordt `number` afgerond naar het `digits` aantal decimale cijfers.
