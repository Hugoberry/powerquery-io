...
A(z) `dateTimeZone` szöveges alakját adja vissza. Megadható egy nem kötelező `record` paraméter (`options`) további tulajdonságok megadásához. `culture` csak örökölt munkafolyamatokhoz használatos. A `record` az alábbi mezőket tartalmazhatja:
...
### Example #1
A `#datetimezone(2010, 12, 31, 01, 30, 25, 2, 0)` konvertálása `text` értékre. *Az eredmény kimenete eltérő lehet az aktuális kulturális környezettől függően.*
```powerquery
