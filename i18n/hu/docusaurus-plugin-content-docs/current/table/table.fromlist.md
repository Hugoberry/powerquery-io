...
Táblára alakít át egy listát (`list`) a nem kötelező felosztó függvény (`splitter`) a lista összes elemére való alkalmazásával. Alapértelmezésben a listát szöveges értékek listájának tekinti, melyek vesszővel vannak elválasztva. A nem kötelező `columns` paraméter lehet az oszlopok száma, az oszlopok listája vagy egy táblatípus. Megadható nem kötelező `default` és `extraValues` paraméter is.
