...
Egy `count` méretű, `start` értéktől induló, `date` típusú értékekből álló listát ad vissza. A megadott `step` növekmény egy `duration` típusú érték, amelyet a függvény hozzáad minden egyes értékhez.
