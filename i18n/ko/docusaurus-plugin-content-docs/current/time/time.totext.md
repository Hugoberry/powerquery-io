...
`time`의 텍스트 표현을 반환합니다. 추가 속성을 지정하기 위해 선택적 `record` 매개 변수 `options`이(가) 제공될 수 있습니다. `culture`은(는) 레거시 워크플로에만 사용됩니다. `record`에는 다음 필드가 포함될 수 있습니다.
...
### Example #1
`#time(01, 30, 25)`을 `text` 값으로 변환합니다. *결과 출력은 현재 문화권에 따라 다를 수 있습니다.*
```powerquery
