...
`url`에서 다운로드한 헤더를 레코드로 반환합니다. 추가 속성을 지정하기 위해 선택적 레코드 매개 변수 `options`을(를) 제공할 수 있습니다. 레코드에는 다음 필드가 포함될 수 있습니다.
...
### Example #1
상대 경로 및 쿼리 옵션을 사용하여 `"https://bing.com/search?q=Power+Query"`에 대한 HTTP 헤더를 검색합니다.
```powerquery
let
