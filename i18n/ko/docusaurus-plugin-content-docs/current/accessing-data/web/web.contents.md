...
`url`에서 다운로드한 콘텐츠를 이진으로 반환합니다. 추가 속성을 지정하기 위해 선택적 레코드 매개 변수인 `options`을(를) 제공할 수 있습니다. 레코드에는 다음 필드가 포함될 수 있습니다.
...
-   `Content` : 이 값을 지정하면 옵션 값을 POST의 내용으로 사용하여 GET에서 POST로의 웹 요청이 변경됩니다.
...
### Example #1
RelativePath 및 쿼리 옵션을 사용하여 `"https://bing.com/search?q=Power+Query"`의 콘텐츠를 검색합니다. 이러한 옵션을 사용하여 정적 기본 URL을 동적으로 쿼리할 수 있습니다.
```powerquery
let
