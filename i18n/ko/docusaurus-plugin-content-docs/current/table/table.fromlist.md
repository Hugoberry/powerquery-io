...
목록의 각 항목에 선택적 분할 함수`splitter`을(를) 적용하여 목록 `list`을(를) 테이블로 변환합니다. 기본적으로 목록은 쉼표로 분할된 텍스트 값 목록으로 간주됩니다. 선택적 `columns`은(는) 열 수, 열 목록 또는 TableType일 수 있습니다. 선택적으로 `default` 및 `extraValues`을(를) 지정할 수도 있습니다.
