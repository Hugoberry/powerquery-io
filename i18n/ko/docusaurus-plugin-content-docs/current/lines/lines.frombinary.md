...
이진 값을 줄 바꿈 시 분할된 텍스트 값 목록으로 변환합니다.

-   `binary`: 목록으로 변환할 이진 값입니다.
...
-   `encoding`: 이진 값의 텍스트 인코딩을 지정합니다. `encoding` 값은 `null`일 수 있습니다. 기본값은 `65001`(UTF-8)입니다.

레코드가 `quoteStyle` 대해 지정되고( `includeLineSeparators` 및 `encoding` `null`)인 경우 다음 레코드 필드를 제공할 수 있습니다.

-   `QuoteStyle`: 따옴표 붙은 줄 바꿈 처리 방법을 지정합니다.
...
-   `CsvStyle`: 따옴표 처리 방법을 지정합니다. `QuoteStyle.None`와(과) 함께 사용할 수없습니다. 안
...
-   `Encoding`: 이진 값의 텍스트 인코딩입니다. 기본값은 `65001`(UTF-8)입니다.
