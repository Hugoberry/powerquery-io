...
Adaugă o nouă coloană `newColumnName` la `table` cu valorile reprezentative `columnName`. Reprezentanții sunt obținuți prin potrivirea vagă a valorilor din `columnName` pentru fiecare rând.

Un set opțional de `options` poate fi inclus pentru a specifica modul de comparare a coloanelor cheie. Opțiunile sunt:
