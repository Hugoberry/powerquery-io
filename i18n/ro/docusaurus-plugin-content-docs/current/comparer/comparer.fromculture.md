...
Returnează o funcție de comparare care utilizează `culture` și sensibilitatea la literele mari și mici specificată de `ignoreCase` pentru a efectua comparații.  
...
Valoarea implicită pentru `ignoreCase` este false. `culture` ar trebui să fie una dintre setările regionale acceptate de .NET Framework (de exemplu, "ro-RO").
