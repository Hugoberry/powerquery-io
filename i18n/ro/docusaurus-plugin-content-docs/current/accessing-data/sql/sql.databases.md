...
Returnează un tabel cu bazele de date pe serverul SQL Server specificat, `server`. Un parametru opțional de înregistrare, `options`, poate fi specificat pentru a controla următoarele opțiuni:
...
Nu acceptă setarea unei interogări SQL care să ruleze pe server. `Sql.Database` trebuie utilizată în schimb pentru a rula o interogare SQL.
