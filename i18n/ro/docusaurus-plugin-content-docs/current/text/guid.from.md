...
Returnează o valoare `Guid.Type` din `value` dat. Dacă `value` dat este `null`, `Guid.From` returnează `null`. Se va efectua o verificare pentru a se stabili dacă `value` dat este într-un format acceptabil. Formatele acceptabile sunt furnizate în exemple.
