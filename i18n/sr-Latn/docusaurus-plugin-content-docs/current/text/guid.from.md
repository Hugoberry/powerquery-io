...
Vraća vrednost `Guid.Type` iz date `value`. Ako je dati `value` jednak `nuli`, `Guid.From` vraća `null`. Biće izvršena promena da bi se utvrdilo da li je `value` u prihvatljivom formatu. Prihvatljivi formati su navedeni u primerima.
