...
Genomför angivna namnbyten på kolumnerna i tabellen `table`. En ersättningsåtgärd, `renames`, består av en lista med två värden, det gamla kolumnnamnet och det nya kolumnnamnet, som anges i en lista. Om kolumnen inte finns genereras ett fel om inte den valfria parametern `missingField` anger ett alternativ (t.ex. `MissingField.UseNull` eller `MissingField.Ignore`).
