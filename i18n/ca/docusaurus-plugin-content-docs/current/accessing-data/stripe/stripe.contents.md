...
Fa una trucada a l'API de l'Stripe API al https://api.stripe.com/v1/`method`, amb el registre `query` opcional que es passa com a paràmetres opcionals i `pageLimit` opcional com a límit del número de sol·licituds de l'API. Primer es retornen les dades més recents. Si no s'especifica cap `pageLimit`, es retornen totes les dades. S'utilitza la versió 2015-10-16 de l'Stripe.
