...
Crea una taula del tipus `columns` a partir d'una llista `lists` que conté llistes imbricades amb els noms i valors de columna. Si algunes columnes tenen més valors que altres, els valors que hi faltin s'emplenaran amb el valor per defecte, "null", si les columnes són anul·lables.
