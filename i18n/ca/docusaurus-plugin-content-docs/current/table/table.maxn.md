...
Retorna les files més grans de `table`, atesos els `comparisonCriteria`. Després que s'ordenin les files, s'ha d'especificar el paràmetre `countOrCondition` per filtrar encara més el resultat. Tingueu en compte que l'algoritme d'ordenació no pot garantir un resultat ordenat fix. El paràmetre `countOrCondition` pot tenir diverses formes:
