...
Zwraca tabelę zawierającą tabele i widoki SQL dostępne w bazie danych programu Informix.
...
Zwraca tabelę zawierającą tabele i widoki SQL dostępne w bazie danych Informix na serwerze `server` w wystąpieniu bazy danych o nazwie `database`. Opcjonalnie można określić port serwera oddzielony dwukropkiem. Za pomocą opcjonalnego parametru rekordu `options` można określić następujące opcje:
