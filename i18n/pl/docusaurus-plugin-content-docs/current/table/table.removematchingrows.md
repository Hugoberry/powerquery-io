...
-   `equationCriteria`: (opcjonalnie) określa sposób, w jaki jest ustalana równość podczas porównywania wartości. Ten parametr może być funkcją selektora klucza, funkcją porównującą lub listą kolumn w tabeli używanych do porównywania wierszy.


## Examples

### Example #1
Usuń wszystkie wiersze, w których \[a = 1\] z podanej tabeli.
```powerquery
Table.RemoveMatchingRows(
    Table.FromRecords({
        [a = 1, b = 2],
...
### Example #2
Usuń anulowane zamówienia, ignorując wielkość liter.
```powerquery
let
...
### Example #3
Usuń wszystkie zadania konserwacji przypadające na święta w USA.
```powerquery
let
