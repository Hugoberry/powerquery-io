...
建立動作，其在執行時會使用 HTTP 作為二進位值傳回針對 `method` 執行 `url` 要求的結果。 可提供選擇性記錄參數 `options` 以指定其他屬性。記錄可以包含下列欄位:
...
-   `Content` : 指定此值會使其內容成為 HTTP 要求的本文。

請注意，此功能在大多數情况下是停用的。請考慮改為使用 Web.Contents 或 Web.Headers。
