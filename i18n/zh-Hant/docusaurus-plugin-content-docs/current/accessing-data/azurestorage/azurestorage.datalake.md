...
從 Azure Data Lake Storage 檔案系統傳回導覽資料表，內含在帳戶 URL `endpoint` 之指定容器及其子資料夾中找到的文件。可指定 `options` 來控制下列選項:
...
-   `HierarchicalNavigation` : 邏輯 (true/false)，控制以樹狀目錄檢視或簡單列表傳回檔案。預設值為 false。
