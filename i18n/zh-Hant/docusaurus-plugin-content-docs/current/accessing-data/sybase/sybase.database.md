...
傳回 Sybase 資料庫中可用的 SQL 資料表和檢視表的資料表。
...
傳回 Sybase 資料庫 (位於伺服器 `server` 上名稱為 `database` 的資料庫執行個體內) 中可用的 SQL 資料表與檢視之資料表。指定伺服器時可選擇性地指定連接埠 (以冒號分隔)。您可指定選擇性的記錄參數 `options` 以控制下列選項:
