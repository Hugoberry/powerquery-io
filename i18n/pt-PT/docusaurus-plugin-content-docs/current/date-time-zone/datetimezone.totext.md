...
Devolve uma representação textual de `dateTimeZone`. Poderá ser fornecido um parâmetro `record` opcional, `options`, para especificar propriedades adicionais. `culture` é utilizado apenas para fluxos de trabalho legados. O `record` pode conter os seguintes campos:
...
### Example #1
Converter `#datetimezone(2010, 12, 31, 01, 30, 25, 2, 0)` num valor de `text`. *O resultado pode variar dependendo da cultura atual.*
```powerquery
