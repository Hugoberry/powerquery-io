...
Devolve uma tabela de bases de dados no SQL Server especificado, `server`. Pode ser especificado um parâmetro de registo opcional, `options`, para controlar as seguintes opções:
...
Não suporta a definição de uma consulta SQL a executar no servidor. Deve utilizar-se `Sql.Database` para executar uma consulta SQL.
