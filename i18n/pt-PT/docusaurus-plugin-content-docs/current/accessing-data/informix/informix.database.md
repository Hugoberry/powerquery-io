...
Devolve uma tabela de tabelas e vistas SQL disponíveis numa base de dados Informix no servidor `server`, na instância de base de dados denominada `database`. A porta pode ser especificada opcionalmente com o servidor, separada por dois-pontos. Pode ser especificado um parâmetro de registo opcional, `options`, para controlar as seguintes opções:
