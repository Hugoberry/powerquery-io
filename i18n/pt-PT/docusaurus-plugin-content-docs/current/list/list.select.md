...
Devolve os valores da lista especificada que correspondem à condição de seleção.

-   `list`: A lista a examinar.
-   `selection`: a função que determina os valores a selecionar.
...
### Example #2
Selecionar as datas da lista que correspondem a sábado ou domingo.
```powerquery
let
    dates = {
...
### Example #3
Apresentar uma tabela de clientes ativos com totais de compra superiores a 100 USD.
```powerquery
let
    customers = {
