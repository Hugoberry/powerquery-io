...
Devolve unha táboa de bases de datos no servidor SQL especificado, `server` . Pódese especificar un parámetro de rexistro opcional, `options` , para controlar as seguintes opcións:
...
Non admite a configuración dunha consulta SQL para que se execute no servidor. No seu lugar, debería usarse `Sql.Database` para executar unha consulta SQL.
