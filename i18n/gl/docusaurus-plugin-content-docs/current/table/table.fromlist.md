...
Converte unha lista, `list`, nunha táboa aplicando a función de división opcional, `splitter`, para cada elemento da lista. Por defecto, a lista asúmese como unha lista de valores de texto dividida por comas. O `columns` opcional pode ser o número de columnas, unha lista de columnas ou un TableType. Tamén se poden especificar os opcionais `default` e `extraValues`.
