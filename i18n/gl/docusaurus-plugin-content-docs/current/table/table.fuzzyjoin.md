...
Une as filas de `table1` coas filas de `table2` baseándose nunha coincidencia parcial dos valores das columnas de clave seleccionadas por `key1` (para `table1`) e `key2` (para `table2`).

A coincidencia parcial é unha comparación baseada na similitude do texto, non na igualdade deste.

Por defecto, execútase unha combinación interna; porén, pódese incluír unha `joinKind` opcional para especificar o tipo de combinación. As opcións inclúen:
...
Pódese incluír un conxunto de `joinOptions` opcional para especificar como comparar as columnas de clave. As opcións inclúen:
