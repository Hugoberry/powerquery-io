...
Devolve un valor `Guid.Type` do valor `value` fornecido. Se o valor `value` fornecido é `null`, `Guid.From` devolve `null`. Realizarase unha comprobación para determinar se o valor `value` fornecido está nun formato aceptable. Os formatos aceptables fornécense nos exemplos.
