...
Tablolar arasında sağlanan sütunlar üzerinde benzer birleşim gerçekleştirir ve birleşim sonucunu yeni bir sütunda oluşturur.
...
`key1` (`table1` için) ve `key2` (`table2` için) tarafından seçilen anahtar sütunlarının değerlerinin benzer eşleştirmesine dayanarak `table1` satırlarını `table2` satırlarıyla birleştirir. Sonuçlar, `newColumnName` adlı yeni bir sütunda döndürülür.

Benzer öğe eşleştirme, metnin eşitliğinden ziyade benzerliğini temel alan bir kıyaslamadır.

İsteğe bağlı `joinKind`, gerçekleştirilecek birleşim türünü belirtir. Bir `joinKind` belirtilmediyse varsayılan olarak bir sol dış birleştirme gerçekleştirilir. Seçenekler şunlardır:
...
Anahtar sütunlarının nasıl kıyaslanacağını belirtmek için isteğe bağlı olarak bir `joinOptions` kümesi eklenebilir. Seçenekler şunlardır:
