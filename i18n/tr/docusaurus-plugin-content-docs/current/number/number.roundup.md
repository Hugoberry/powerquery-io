...
`number` sayısının yukarı yuvarlama sonucunu sonraki en yüksek tamsayıya döndürür. `number` null ise bu işlev null değerini döndürür. `digits` belirtilmişse `number` belirtilen ondalık basamak sayısına yuvarlanır.
