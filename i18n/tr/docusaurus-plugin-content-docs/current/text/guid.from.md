...
Verilen `value` değerinden bir `Guid.Type` değeri döndürür. Verilen `value` `null` olduğunda `Guid.From`, `null` değerini döndürür. Verilen `value` değerinin kabul edilebilir bir biçimde olup olmadığını saptamak için bir denetim gerçekleştirilecektir. Kabul edilebilir biçimler örneklerde verilmiştir.
