...
Gruppiert die Zeilen von "`table`", indem für jede Zeile Fuzzyübereinstimmungen der Werte in der angegebenen Spalte "`key`" gesucht werden. Für jede Gruppe wird ein Datensatz erstellt, der die Schlüsselspalten (und ihre Werte) zusammen mit den von "`aggregatedColumns`" angegebenen aggregierten Spalten enthält. Diese Funktion kann nicht garantieren, dass die Zeilen in einer bestimmten Reihenfolge zurückgegeben werden.
...
### Example #1
Gruppiert die Tabelle und fügt eine aggregierte Spalte \[Count\] hinzu, die die Anzahl der Mitarbeiter an jedem Standort enthält (`each Table.RowCount(_)`).
```powerquery
Table.FuzzyGroup(
    Table.FromRecords(
        {
