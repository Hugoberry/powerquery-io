...
Restituisce una tabella di tabelle e viste SQL dal database Oracle.
...
Restituisce una tabella di tabelle e viste SQL dal database Oracle nel server `server`. Facoltativamente, è possibile specificare la porta con il server, separando le voci con i due punti. È possibile specificare un parametro di record facoltativo, `options`, per controllare le seguenti opzioni:
