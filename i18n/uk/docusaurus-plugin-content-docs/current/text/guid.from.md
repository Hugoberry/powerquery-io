---
title: Guid.From
---

# Guid.From


Повертає значення GUID для заданого значення.


## Syntax

```powerquery
Guid.From(
    value as text
) as text
```


## Remarks

Повертає значення типу `Guid.Type` для заданого значення "`value`". Якщо значення "`value`" задано як `null`, функція `Guid.From` повертає `null`. Система перевірятиме, чи значення "`value`" задано в прийнятному форматі. Прийнятні формати наведено в прикладах.


## Examples

### Example #1
GUID можна надавати як 32 шістнадцяткові цифри без роздільників.
```powerquery
Guid.From("05FE1DADC8C24F3BA4C2D194116B4967")
```

Result: 
```powerquery
"05fe1dad-c8c2-4f3b-a4c2-d194116b4967"
```


### Example #2
GUID можна надавати як 32 шістнадцяткові цифри, розділені дефісами на блоки 8-4-4-4-12.
```powerquery
Guid.From("05FE1DAD-C8C2-4F3B-A4C2-D194116B4967")
```

Result: 
```powerquery
"05fe1dad-c8c2-4f3b-a4c2-d194116b4967"
```


### Example #3
GUID можна надавати як 32 шістнадцяткові цифри, розділені дефісами та взяті у фігурні дужки.
```powerquery
Guid.From("{05FE1DAD-C8C2-4F3B-A4C2-D194116B4967}")
```

Result: 
```powerquery
"05fe1dad-c8c2-4f3b-a4c2-d194116b4967"
```


### Example #4
GUID можна надавати як 32 шістнадцяткові цифри, розділені дефісами та взяті в дужки.
```powerquery
Guid.From("(05FE1DAD-C8C2-4F3B-A4C2-D194116B4967)")
```

Result: 
```powerquery
"05fe1dad-c8c2-4f3b-a4c2-d194116b4967"
```




## Category
Text.Conversions from and to text
