...
Перетворює список `list` у таблицю, застосовуючи додаткову функцію розділення `splitter` до кожного пункту у списку. За замовчуванням, список вважається списком текстових значень, який розділяється комами. Додатковим параметром `columns` може бути кількість стовпців, списком стовпців або TableType. Можна також вказати додаткові параметри `default` і `extraValues`.
