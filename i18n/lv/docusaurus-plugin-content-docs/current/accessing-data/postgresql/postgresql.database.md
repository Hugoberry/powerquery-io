...
Tiek atgriezta servera `server` PostgreSQL datu bāzē pieejamo SQL tabulu un skatu tabula datu bāzes instancē `database`. Kopā ar serveri pēc izvēles var norādīt portu, atdalot to ar kolu. Lai pārvaldītu tālāk minētās opcijas, var norādīt neobligātu ieraksta parametru `options` Ieraksta parametrs tiek norādīts šādi:
