...
Atgriež `Guid.Type` vērtību atbilstoši dotajai vērtībai `value`. Ja dotā vērtība `value` ir `Null`, `Guid.From` atgriež `Null`. Tiks veikta pārbaude, lai noskaidrotu, vai dotā vērtība `value` ir pieņemamā formātā. Piemēros ir norādīti pieņemami formāti.
