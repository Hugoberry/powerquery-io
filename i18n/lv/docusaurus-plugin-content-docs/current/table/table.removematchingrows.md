...
-   `table`: tabula, kur meklēt.
-   `rows`: saraksts, kurā ir informācija par noņemamajām rindām.
-   `equationCriteria`: (neobligāti) norāda, kā, salīdzinot vērtības, tiek noteikta vienādība. Šis parametrs var būt taustiņu atlasītāja funkcija, salīdzināšanas funkcija vai tabulas kolonnu saraksts, kas jālieto, salīdzinot rindas.


## Examples

### Example #1
Noņemiet visas rindas, kur \[a = 1\] no norādītās tabulas.
```powerquery
Table.RemoveMatchingRows(
    Table.FromRecords({
        [a = 1, b = 2],
...
### Example #2
Noņemt atceltos pasūtījumus, ignorējot lietu.
```powerquery
let
...
### Example #3
Noņemiet visus uzturēšanas uzdevumus, kas iekrīt ASV svētku dienās.
```powerquery
let
