...
Возвращает таблицу, не содержащую указанное число первых строк (`countOrCondition`) в таблице `table`. Количество пропущенных строк зависит от необязательного параметра `countOrCondition`.

-   Если `countOrCondition` не указано, пропускается только первая строка.
-   Если `countOrCondition` — число, пропускается соответствующее количество строк (с начала).
