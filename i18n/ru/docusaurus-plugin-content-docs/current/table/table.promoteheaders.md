...
Назначает первую строку значений в качестве новых заголовков столбцов (т. е. имен столбцов). По умолчанию заголовками назначаются только текстовые или числовые значения. Допустимые варианты:

-   `PromoteAllScalars`: если задано значение `true`, все скалярные значения в первой строке назначаются заголовками с использованием языка и региональных параметров (`Culture`), если таковой указан (или текущего языкового стандарта документа). Для значений, которые невозможно преобразовать в текст, будет использоваться имя столбца по умолчанию.
-   `Culture`: название языка и региональных параметров, задающее язык и региональные параметры для данных.
