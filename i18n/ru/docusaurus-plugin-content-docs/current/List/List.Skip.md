...
Возвращает список, который пропускает первый элемент списка <code>list</code>. Если список <code>list</code> пуст, возвращается пустой список.Эта функция принимает необязательный параметр <code>countOrCondition</code> для поддержки пропуска нескольких значений, как указано ниже. <ul> <li>Если указано число, пропускается количество элементов до указанного значения. </li> <li>Если указано условие, возвращаемый список начинается с первого элемента в <code>list</code>, удовлетворяющего условию. Как только обнаруживается элемент, не соответствующий условию, последующие элементы не рассматриваются. </li> <li>Если параметр имеет значение NULL, наблюдается поведение по умолчанию. </li> </ul>
