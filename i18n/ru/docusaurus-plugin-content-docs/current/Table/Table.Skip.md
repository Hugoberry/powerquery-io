...
Возвращает таблицу, не содержащую указанное число первых строк (<code>countOrCondition</code>) в таблице <code>table</code>.    Количество пропущенных строк зависит от необязательного параметра <code>countOrCondition</code>.    <ul>    <li> Если <code>countOrCondition</code> не указано, пропускается только первая строка. </li>    <li> Если <code>countOrCondition</code> — число, пропускается соответствующее количество строк (с начала). </li>    <li> Если <code>countOrCondition</code> — условие, будут пропускаться строки, соответствующие условию, до первой строки, не соответствующей условию.</li>    </ul>
