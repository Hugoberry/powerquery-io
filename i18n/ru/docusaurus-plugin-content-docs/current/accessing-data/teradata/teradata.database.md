...
Возвращает список таблиц и представлений SQL из базы данных Teradata на сервере `server`. Дополнительно к имени сервера можно указать порт (через двоеточие). Можно указать необязательный параметр записи, `options`, для управления следующими вариантами:
