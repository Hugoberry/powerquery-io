...
Вызывает Stripe API по адресу https://api.stripe.com/v1/`method` с необязательной записью `query`, передаваемой в качестве дополнительных параметров, и необязательной записью `pageLimit` в качестве ограничения на количество запросов API. Сначала возвращаются самые новые данные. Если значение `pageLimit` не указано, возвращаются все данные. Используется версия Stripe 2015-10-16.
