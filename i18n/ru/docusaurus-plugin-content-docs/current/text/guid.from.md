...
Возвращает значение `Guid.Type`, полученное из предоставленного значения `value`. Если данное `value` равно `NULL`, то `Guid.From` возвращает `NULL`. Данное `value` проверяется на соответствие приемлемому формату. Эти форматы приведены в примерах.
